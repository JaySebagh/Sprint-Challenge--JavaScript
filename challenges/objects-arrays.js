...
/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that
contains them all. Log the result. */
let uni = [];

let inc = "Uni"

for (let i = 0; i < graduates.length; i++) {
  if (graduates[i].university.includes(inc)) {
...
zooAnimals.forEach(function(newNames) {
  let bothNames = `Name: ${newNames.animal_name}, Scientific: ${newNames.scientific_name}`;
...
lowerCase = zooAnimals.map(function(newNames2) {
  let firstNames = newNames2.animal_name.toLowerCase();
  return firstNames
});
...
largerPopulation = zooAnimals.filter(five => five.population < 5);
...
zooAnimals.forEach(function(newPop) {
  let thePop = newPop.population;
  populationTotal.push(thePop)
})

populationTotal = populationTotal.reduce(function(total, current) {
